...
namespace v2rayN.Forms
{
    public partial class RoutingRuleSettingForm : BaseForm
    {
        ...
        private void InitRoutingsView()
        {
            ...
            lvRoutings.Columns.Add("outboundTag", 200);
            ...
            lvRoutings.Columns.Add("boundIndexId", 80);
        ...
        private void RefreshRoutingsView()
        {
            ...
            foreach (var item in routingItem.rules)
            {
                ...
                if (outboundTag.Equals("proxy"))
                {
                    item.boundIndexId = config.indexId;

                }
                else if (outboundTag.Equals("direct") || outboundTag.Equals("block")) { }
                else
                {
                    int index = config.FindIndexId(item.boundIndexId);
                    if(index < 0)
                    {
                        outboundTag = "[proxy]unkown";
                    }
                    else
                    {
                        outboundTag = "[proxy]" + config.vmess[index].remarks;
                    }
                }

                ListViewItem lvItem = new ListViewItem("");
                Utils.AddSubItem(lvItem, "outboundTag", outboundTag);
                ...
                Utils.AddSubItem(lvItem, "boundIndexId", item.boundIndexId);
