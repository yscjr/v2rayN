...
namespace v2rayN.Handler
{
    ...
    class V2rayConfigHandler
    {
        ...
        /// <param name="v2rayConfig"></param>
        /// <returns></returns>
        private static int outbound(VmessItem node, ref V2rayConfig v2rayConfig, int index = 0)
        {
            try
            {
                var config = LazyConfig.Instance.GetConfig();
                Outbounds outbound = v2rayConfig.outbounds[index];
        ...
        public static int GenerateClientConfigContent(VmessItem node, bool blExport, ref V2rayConfig v2rayConfig, out string msg)
        {
            try
            {
                ...
                //outbound
                Outbounds outboundsOrigin = Utils.DeepCopy(v2rayConfig.outbounds[0]);
                ...
                foreach (var item in config.routings[config.routingIndex].rules)
                {
                    string outboundTag = item.outboundTag;
                    if (outboundTag.Equals("proxy") || outboundTag.Equals("direct") || outboundTag.Equals("block"))
                    {
                        continue;
                    }
                    var index = config.FindIndexId(item.boundIndexId);
                    if (index < 0)
                    {
                        continue;
                    }
                    var vmessItem = config.vmess[index];
                    outboundsOrigin.tag = $"proxy{index + 1}";
                    v2rayConfig.outbounds.Add(Utils.DeepCopy(outboundsOrigin));
                    //v2rayConfig.outbounds.Insert(0, Utils.DeepCopy(outbounds2));
                    outbound(vmessItem, ref v2rayConfig, i++);
                }
